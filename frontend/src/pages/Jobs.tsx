...
import { PlayIcon, ClockIcon, CheckCircleIcon, XCircleIcon, ArrowPathIcon } from '@heroicons/react/24/outline'
...
export default function Jobs() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <span className="text-dark-400">{status}</span>
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-dark-400 mt-1">Monitor and manage scheduled tasks</p>
      </div>
      ...
      <div className="bg-dark-800 rounded-xl border border-dark-700 shadow-lg">
        ...
        <div className="p-6">
          {jobsLoading ? (
            <div className="text-center py-8 text-dark-400">Loading jobs...</div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-dark-400 font-mono">{job.id}</span>
                      </div>
            ...
            <div className="text-center py-8 text-dark-400">No jobs configured</div>
          )}
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-dark-700/50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-dark-300 uppercase tracking-wider">Job</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-dark-300 uppercase tracking-wider">Status</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-dark-300 uppercase tracking-wider">Started</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-dark-300 uppercase tracking-wider">Duration</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-dark-300 uppercase tracking-wider">Details</th>
              </tr>
            </thead>
              ...
              {executionsLoading ? (
                ...
                <tr key={exec.id} className="hover:bg-dark-700/30">
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-white">{exec.job_name}</div>
                    <div className="text-xs text-dark-400 font-mono">{exec.job_id}</div>
                  </td>
                  <td className="px-6 py-4">{getStatusBadge(exec.status)}</td>
                  <td className="px-6 py-4 text-sm text-dark-300">{formatDateTime(exec.started_at)}</td>
                  <td className="px-6 py-4 text-sm text-dark-300">{formatDuration(exec.duration_seconds)}</td>
                  <td className="px-6 py-4">
                    {exec.error_message ? (
                      ...
                      <div className="text-xs text-dark-400">
                        {Object.entries(exec.details).slice(0, 2).map(([key, value]) => (
                          <div key={key}>
                            {key}: <span className="text-dark-300">{JSON.stringify(value)}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
